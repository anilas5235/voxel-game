...
namespace Voxels.Mesher
{
    ...
    public static class GreedyMesher
    {
            ...
            for (int direction = 0; direction < 3; direction++)
            {
                ...
                for (chunkItr[direction] = -1; chunkItr[direction] < mainAxisLimit;)
                {
                    ...
                    for (chunkItr[axis2] = 0; chunkItr[axis2] < axis2Limit; ++chunkItr[axis2])
                    {
                        ...
                        {
                            int currentBlock = accessor.GetBlockInChunk(pos, chunkItr);
                            int compareBlock = accessor.GetBlockInChunk(pos, chunkItr + directionMask);

                            byte currentMeshIndex = GetMeshIndex(currentBlock);
                            byte compareMeshIndex = GetMeshIndex(compareBlock);
                            ...
                            {
                                normalMask[n++] = new Mask(currentBlock, currentMeshIndex, 1,
                                    ComputeAOMask(accessor, pos, chunkItr + directionMask, axis1, axis2));
                            }
                            else
                            {
                                normalMask[n++] = new Mask(compareBlock, compareMeshIndex, -1,
                                    ComputeAOMask(accessor, pos, chunkItr, axis1, axis2));
                            }
                        }
                    }
                        ...
                        for (int i = 0; i < axis1Limit;)
                        {
                            if (normalMask[n].Normal != 0)
                            {
                                ...
                                for (height = 1; j + height < axis2Limit; height++)
                                {
                                    ...
                                    {
                                        if (CompareMask(normalMask[n + k + height * axis1Limit], currentMask))
                                            continue;
        ...
        )
        {
            ...
            int uvz = GetUV0Index(mask.Block, normal);
