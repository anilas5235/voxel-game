...
namespace Voxels.Chunk {
    ...
    internal struct ChunkAccessor {
        ...
        private NativeParallelHashMap<int3, ChunkData>.ReadOnly _Chunks;
        private int3 _ChunkSize;

        internal ChunkAccessor(NativeParallelHashMap<int3, ChunkData>.ReadOnly chunks, int3 chunkSize) {
            _Chunks = chunks;
        ...
        internal int GetBlockInChunk(int3 chunk_pos, int3 block_pos) {
            ...
            return TryGetChunk(chunk_pos + key, out var chunk) ? chunk.GetVoxel(block_pos) : 0;
        }

        internal bool TryGetChunk(int3 pos, out ChunkData chunk) => _Chunks.TryGetValue(pos, out chunk);

        internal bool ContainsChunk(int3 coord) => _Chunks.ContainsKey(coord);

        #region Try Neighbours
        
        internal bool TryGetNeighborPX(int3 pos, out ChunkData chunk) {
            var px = pos + new int3(1,0,0) * _ChunkSize;

            return _Chunks.TryGetValue(px, out chunk);
        }

        internal bool TryGetNeighborPY(int3 pos, out ChunkData chunk) {
            var py = pos + new int3(0,1,0) * _ChunkSize;

            return _Chunks.TryGetValue(py, out chunk);
        }

        internal bool TryGetNeighborPZ(int3 pos, out ChunkData chunk) {
            var pz = pos + new int3(0, 0, 1) * _ChunkSize;

            return _Chunks.TryGetValue(pz, out chunk);
        }

        internal bool TryGetNeighborNX(int3 pos, out ChunkData chunk) {
            var nx = pos + new int3(-1,0,0) * _ChunkSize;

            return _Chunks.TryGetValue(nx, out chunk);
        }

        internal bool TryGetNeighborNY(int3 pos, out ChunkData chunk) {
        ...
        internal bool TryGetNeighborNZ(int3 pos, out ChunkData chunk) {
